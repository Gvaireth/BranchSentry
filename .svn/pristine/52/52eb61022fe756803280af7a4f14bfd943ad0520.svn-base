...
public class BranchStatisticsFactory {
	public static BranchStatistics generate(BranchContent branchContent) {
		...
		addTopCommentLength(branchContent, stats);
	...
	private static void addTopCommentLength(BranchContent branchContent, BranchStatistics stats) {
		int maxCommentLength = 0;
		for (LogEntry logEntry : branchContent.getLogEntries()) {
			if (logEntry.getMsg().length() > maxCommentLength)
				maxCommentLength = logEntry.getMsg().length();
		}
		stats.setTopCommentLength(maxCommentLength);
	}
